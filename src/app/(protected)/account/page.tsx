...
export default function AccountPage() {
  ...
  const { data, isLoading, error, refetch } =
    api.onboarding.createUser.useQuery(undefined, {
      retry: 1,
      refetchOnWindowFocus: false,
    });

  const { mutateAsync: updatePreparingFor } =
    api.onboarding.updatePreparingFor.useMutation({
      onSuccess: () => {
  ...
  const { mutateAsync: updateCountryName } =
    api.onboarding.updateCountryName.useMutation({
      onSuccess: () => {
          ...
          <TabsContent value="profile" className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Manage your personal information and how it appears on the
                  platform
                </CardDescription>
              </CardHeader>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="space-y-2">
                      ...
                      >
                        ...
                        <SelectContent>
                          <SelectItem value="school_student">
                            School Student
                          </SelectItem>
                          <SelectItem value="college_student">
                            College Student
                          </SelectItem>
                          <SelectItem value="preparing_for_exam">
                            Preparing for Exam
                          </SelectItem>
                        </SelectContent>
                      </Select>
                        ...
                        <Input
                          ...
                          onKeyDown={(e) => {
                            if (e.key === "Enter" || e.key === ",") {
                              ...
                              if (
                                trimmedValue &&
                                !formData.preparing_for.includes(trimmedValue)
                              ) {
                                const newItems = [
                                  ...formData.preparing_for,
                                  trimmedValue,
                                ];
                                setFormData((prev) => ({
                                  ...prev,
                                  preparing_for: newItems,
                                }));
                        ...
                        {formData.preparing_for.length > 0 && (
                          <div className="flex flex-wrap gap-2 mt-2">
                            {formData.preparing_for.map((item, index) => (
                              <div
                                key={index}
                                className="bg-[#444] text-white px-3 py-1 rounded-full flex items-center"
                              >
